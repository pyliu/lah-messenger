<template lang="pug">
  .chat-channel-list(v-if="isChat")
    ...
    b-avatar-group(size="3rem" :overlap="overlapRatio"): transition-group.d-flex.justify-content-center.flex-wrap(name="listY" mode="out-in"): user-avatar(
      v-for="(user, idx) in connectedUsers"
      :key="`avatar_${user.userid}_${idx}`"
      :user-data="user"
      :class="connectedUsersCount <= 60 ? ['m-1'] : []"
    )
</template>
